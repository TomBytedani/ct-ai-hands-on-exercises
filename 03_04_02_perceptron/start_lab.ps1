...
$environmentPath = Join-Path $environmentRoot 'lab-3-4-2'
...
if ($LASTEXITCODE -ne 0) { throw 'The lab dependencies could not be installed.' }

$jupyterArguments = @('-m', 'jupyter', 'lab', $notebookPath, "--ServerApp.root_dir=$labRoot")
if ($NoBrowser) { $jupyterArguments += '--no-browser' }
